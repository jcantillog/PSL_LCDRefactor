...
public class LCDPrinter {
    ...
    private void imprimirNumero(int size, String numeroImp, int espacio) {
        ...
        for (int i = 0; i < this.totalFilas; i++) {
            for (int j = 0; j < this.totalColum; j++) {
                LCDDisplay.show_number(this.matrizImpr[i][j]);
            }
            LCDDisplay.show_number("\n");
        }
    }
    ...
    public void procesar(String comando, int espacioDig) {
        ...
        if (isNumeric(parametros[0])) {
            ...
            if (!rangeOk(OPTION, parametros[0])) {
    ...
    static boolean rangeOk(int option, String cadena){
    ...
    static boolean rangeOk(int numero){
